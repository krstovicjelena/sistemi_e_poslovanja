...
export class EmployeeController{
    ...
    @Get(':id')
    getById(@Param('id') employeeId:number ):Promise<Employee> {
    ...
    @Put()
    add(@Body() data:AddEmployeeDto): Promise<Employee>{
        return this.employeeService.add(data);

    }
    //POST localhost:3000/api/employee/4
    @Post(':id')
    edit(@Param('id') id:number, @Body() data:EditEmployeeDto):Promise<Employee>{
